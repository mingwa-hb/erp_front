      ...
      <a-col :md="24" :sm="24">
        <a-card :bordered="false">
          ...
          <a-input
            ...
            @pressEnter="onSearch"
          ></a-input>
          <a-button @click="onSearch" style="margin-left: 20px" icon="search">查询</a-button>
...
export default {
  ...
  components: {},
  props: ['modalWidth', 'multi', 'userIds', 'roleCode', 'visible'],
  data() {
    return {
      queryParam: {
        username: undefined
  ...
  methods: {
    ...
    async loadData(arg) {
      ...
      if (this.selectedDepIds && this.selectedDepIds.length > 0) {
        ...
        params.roleCodeList = this.roleCode
        await ajax
          .appointUserList(params)
          .then(res => {
            if (res.success) {
              console.log(res)
