  ...
  <a-card :bordered="false">
    ...
    >
      ...
      <a-table-column title="操作" dataIndex="" align="center" :width="250" fixed="right">
        <template slot-scope="t, r, i">
          <a href="javascript:void(0);" v-if="r.status != 1" @click="editStatus(1, r)" style="color: rgb(144,96,255);"
            >启用</a
          >
      ...
      <a-form :form="editForm" v-if="editObj.visible">
        ...
        <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="关联表单">
          <a-select
            ...
            v-decorator="[
              'routeName',
              { initialValue: editObj.routeName, rules: [{ required: true, message: '不能为空' }] }
            ]"
          >
            <a-select-option value="">请选择</a-select-option>
            <a-select-option v-for="(item, i) in allFormComponent" :key="i" :value="item.routeName">
    ...
    editObjOk() {
      ...
      this.editForm.validateFields((err, values) => {
        if (!err) {
          ...
          this.postFormAction(this.url.updateInfo, formData)
            .then(res => {
              if (res.success) {
                _this.$message.success('操作成功')
                _this.loadData()
