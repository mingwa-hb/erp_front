...
export default {
  ...
  methods: {
    ...
    changeBackTask(v) {
      ...
      this.userLoading = true;
      console.log("_______"+ JSON.stringify(this.lcModa.processData )  );
      let tableId = this.lcModa.processData.tableId;
      let tableName = this.lcModa.processData.tableName;
      this.getAction(this.url.getNode+"?nodeId="+v+"&tableName="+tableName+"&tableId="+tableId).then(res => {
        this.userLoading = false;
        if (res.success) {
          if (res.result.users && res.result.users.length > 0) {
            this.assigneeList = res.result.users;
