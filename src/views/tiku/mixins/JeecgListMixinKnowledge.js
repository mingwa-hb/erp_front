...
export const JeecgListMixin = {
  ...
  methods: {
    loadData(arg) {
      ...
      this.loading = true;
      getAction(this.url.list + '?parent=0', params).then((res) => {
        if (res.success) {
          //update-begin---author:zhangyafei    Date:20201118  for：适配不分页的数据列表------------
          this.dataSource = res.result.records || res.result;

          for (let i = 0; i < this.dataSource.length; i++) {
            if (!this.dataSource[i].hasChild) {
    ...
    onSelectChange(selectedRowKeys, selectionRows) {
      ...
      this.selectionRowsArr = []
      for (let i = 0; i < selectionRows.length; i++) {
        this.selectionRowsArr.push(selectionRows[i].isApplyExam)
      }
    },
    onClearSelected() {
    ...
    searchQuery() {
      ...
      getAction(this.url.searchKnowledgesByKeyword,data).then(res=>{
        console.log(res);
      })
    },
    superQuery() {
    ...
    handleAdd: function (workTypeId) {
      if (!!this.queryParam.gongzhong && !!this.queryParam.Level) {
        this.$refs.modalForm.add(workTypeId);
        this.$refs.modalForm.title = "新增";
        this.$refs.modalForm.disableSubmit = false;
      } else {
        this.$message.warning('请先选择工种等级！');
      }
    },
    ...
    handleExportXls(fileName) {
      ...
      let workLevel = this.queryParam.Level;
      let workType = this.queryParam.gongzhong;
      let workTypeId = this.id;
      if (this.selectedRowKeys && this.selectedRowKeys.length > 0) {
        param['selections'] = this.selectedRowKeys.join(",")
      }
      console.log("导出参数", param)
      downFile(this.url.exportXlsUrl + "?workLevel=" + workLevel + '&workType=' + workType + '&workTypeId=' + workTypeId, param).then((data) => {
        if (!data) {
          this.loading = false
