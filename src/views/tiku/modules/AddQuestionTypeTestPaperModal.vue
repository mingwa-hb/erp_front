  ...
  >
    ...
    >
      ...
      <a-form-model-item label="难易程度" prop="hardLevelId">
        <a-select placeholder="请选择试题难易程度" v-model.trim="form.hardLevelId">
          <a-select-option
...
export default {
  name: 'AddQuestionTypeTestPaperModal', //组卷弹框-添加|修改题型数量
  mixins: [mixin_tiku],
  components: {},
  props: {
    questionMaxNum: {
  ...
  computed: {
    ...
    hardLevelDisabled() {
      let res = false
      // res = this.form.questionTypeId == 1 && this.form.hardLevelId == 0 ? true : false
      return res
    },
    curMaxNum() {
