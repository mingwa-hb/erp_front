<template>
  <a-modal
    ...
    @cancel="handleCancel"
  >
    <a-card class="page-container" :bordered="false" v-if="!!visible">
      <!--操作-->
              ...
              >
                <a-list-item slot="renderItem" key="item.id" slot-scope="item, index">
                  <TestQuestionSelectItem
                    :init-data="item"
                    @doReplace="hanldeAfterReplace"
                    list-item-type="replace"
...
export default {
  name: 'ReplaceQuestionListModal', //换题弹框
  mixins: [mixin_tiku],
  components: {
  ...
  watch: {
    ...
    targetQuestionItem: {
      ...
      handler(newV, oldV) {
        if (!!newV) {
          this.queryParams.questionType = newV.questionType
          this.refreshList()
        }
      }
    },
  ...
  methods: {
    ...
    hanldeAfterReplace(data) {
      this.selectedRow = data
    },
    hanledReplaceSubmit() {
      let opts = {
        id: this.targetQuestionItem.id,
        questionId: this.selectedRow.id
      }
      this.replaceQuestion(opts)
    },
    formatListData() {
      let res = []
    ...
    replaceQuestion(opts) {
      if (!!opts.id && !!opts.questionId) {
        tikuApiList.replaceTpQuestion(opts).then(res => {
          if (res.code == 200 && !!res.success) {
            this.$message.success(res.message || '替换成功！')
            setTimeout(() => {
              this.$emit('renovate')
              this.handleCancel()
            }, 0)
          } else {
            this.$message.error(res.message || '替换失败！')
          }
        })
      } else {
        this.$message.error('参数ID或questionId不存在，无法提交！')
      }
    },
    getListData() {
      let opts = {
